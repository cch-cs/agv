...
public:
    Wheel(const KDL::Tree& tree,
          const string& agv_base_link, const string& steer_link,
...
private:
    void initPos(const KDL::Tree& tree, const string& agv_base_link);
...
// Initialize pos_.
void Wheel::initPos(const KDL::Tree& tree, const string& agv_base_link)
{
    KDL::Chain chain;
    if (!tree.getChain(agv_base_link, steer_link_, chain))
    {
        throw runtime_error("No kinematic chain was found from \"" + agv_base_link +
                            "\" to \"" + steer_link_ + "\".");
    }
    ...
    if (solver.JntToCart(joint_positions, frame) < 0)
    {
        throw runtime_error("The position of steering link \"" + steer_link_ +
                            "\" in base link frame \"" + agv_base_link +
                            "\" was not found.");
    }
...
const string SteeredWheelBaseController::DEF_BASE_LINK = "agv_base_link";  // NOLINT(runtime/string)
...
const string SteeredWheelBaseController::DEF_BASE_FRAME = "agv_base_link";  // NOLINT(runtime/string)
     ...
     NodeHandle& ctrlr_nh)
{
    ...
    string agv_base_link;
    ctrlr_nh.param("agv_base_link", agv_base_link, DEF_BASE_LINK);
    double timeout;
    ...
    for (int i = 0; i < wheel_param_list.size(); i++)
    {
        ...
        wheels_.push_back(Wheel(model_tree, agv_base_link, steer_link,
